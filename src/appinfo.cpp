...
#include "src/hidapi/hidapi/hidapi.h"
#include "src/database/maindb.h"
#include "src/builddetails.h"
...
QString AppInfo::basicInfoReport(void)
{
    ...
    report += "<tr><td width=300 align=left>Max. elapsed time between device polling: " + QString::number(maxFanControllerPollTime()) + " ms</td></tr>";
    report += "</table>";
    report += "<p></p>";
    report += "<table border=0>";
...
{
    return BuildDetails::versionStr().trimmed() + " " + BuildDetails::buildDateTimeStr();
}
...
QStringList AppInfo::hidDevices(void)
{
    ...
    for(devs = curr_dev = hid_enumerate(0, 0); curr_dev; curr_dev = curr_dev->next) {
