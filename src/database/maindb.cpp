...
NameAndControlModeList MainDb::profileNamesAndModes(void)
{
    ...
    if(!query.exec(QLatin1String("select name, isSoftwareAuto, isAuto from Profile"))) {
        m_lastSqlError = query.lastError();
        return profileList; // return empty list
    }
...
QString MainDb::profileDescription(const QString& profileName)
{
    ...
    bool ok = qry.prepare("select description from Profile where name = :pName;");
    ...
    bool isSoftwareAuto)
{
    ...
    */
    ...
    ok = qry.prepare("insert into Profile (name, description, isAuto, isCelcius, isAudibleAlarm, isSoftwareAuto)"
...
{
    QString dbPathAndName = DatabaseManager::dbFilenameWithPath(DatabaseManager::PrimaryDb);
