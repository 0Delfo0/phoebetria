...
#include "src/gui/gui_mainwindow.h"
#include "src/phoebetriaapp.h"
#include "src/bfx-recon/fancontrollerio.h"
#include "src/gui/gui_about.h"
#include "src/gui/gui_preferences.h"
#include "src/fan/fanprofiles.h"
#include "src/gui/gui_softwareautosetup.h"
#include "src/gui/gui_profiles.h"
#include "src/gui/gui_setmanualrpm.h"
#include "src/database/maindb.h"
#include "src/appinfo.h"
#include "src/gui/gui_diagnostic.h"
#include "src/gui/gui_help.h"
...
void gui_MainWindow::updateSpeedControl(int channel, int RPM, bool updateSlider)
{
    ...
    QString RpmText;
    RpmText = RPM == 0 ? "OFF" : (RPM == RECON_MAXRPM ? QString::number(fcdata().lastRPM(channel)) : QString::number(RPM));
    m_ctrls_currentRPM[channel]->setText(RpmText);

    if(updateSlider || RPM == RECON_MAXRPM) {
        int newRPM;
        if(!fcdata().isAuto() && fcs.isSet_manualRPM() && fcs.manualRPM() != RECON_MAXRPM) {
...
void gui_MainWindow::updateTargetRpmOverlay(int channel)
{
    ...
    if(fcdata().isAuto()) {
        ...
        if(fcdata().isManualRpmSet(channel) && fcdata().lastRPM(channel) != fcdata().manualRPM(channel) && fcdata().manualRPM(channel) != RECON_MAXRPM) {
...
void gui_MainWindow::userChangedChannelRpmSlider(int channel, int value)
{
    ...
    if(!m_ctrls_RpmSliders[channel]->isSliderDown() && (val != fcdata().manualRPM(channel) || val == 0)) {
        fcdata().updateManualRPM(channel, val, false);
        FanControllerIO* fc = &ph_fanControllerIO();
        fc->setChannelSettings(channel, fcdata().alarmTemp(channel), val);
        updateSpeedControl(channel, val, true);
